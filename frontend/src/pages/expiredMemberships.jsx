...
const ExpiredMemberships = () => {
  ...
  }, [currentPage, fetchExpiredMemberships]); // Add fetchExpiredMemberships as dependency
  ...
  const fetchExpiredMemberships = async () => {
    try {
      ...
      const response = await axios.get(`${config.apiUrl}/member/expired?skip=${skip}&limit=${limit}`, {
        headers: {
      ...
      if (response.data.success) {
        setMembers(response.data.expiredMembers || []);
        setTotalMembers(response.data.expiredMembersCount || 0);
        
        const pages = Math.ceil((response.data.expiredMembersCount || 0) / limit);
        setTotalPages(pages);
